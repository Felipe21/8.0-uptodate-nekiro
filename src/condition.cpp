...
bool ConditionDamage::startCondition(Creature* creature)
{
	...
	if (!delayed) {
		...
		if (initDamage == 0) { //agregado fields como rl
		//g_game.addMagicEffect(creature->getPosition(), CONST_ME_BLOCKHIT); //AGREGADO FIELDS COMO RL
			return false;
		}
		/*if (doDamage == 0) { //agregado fields como rl
			g_game.addMagicEffect(creature->getPosition(), CONST_ME_BLOCKHIT); //AGREGADO FIELDS COMO RL
			return false;
		}*/

		// deal initial damage//agregado fields como rl
...
bool ConditionDamage::doDamage(Creature* creature, int32_t healthChange)
{
	...
	//AGREGADO OLD BEHAVIOR WHEN WALKING ON FIELDS AND PLAYER OR MONSTER IS INMUNE
	...
	if (field && creature->getPlayer() && attacker && attacker->getPlayer()) {
		...
		damage.primary.value = static_cast<int32_t>(std::round(damage.primary.value / 1.));// fix double damage fields in pvp
	}
...
void ConditionDamage::addCondition(Creature* creature, const Condition* condition)
{
	...
	initDamage = conditionDamage.initDamage; //agregado fields como rl
	...
	if (init()) {
		...
		if (!delayed) {
			...
			//if (getNextDamage(damage)) {
				//doDamage(creature, damage);
			if (initDamage != 0) {
				doDamage(creature, initDamage); //agregado fields como tibia rl
			}
		}
	}
}
...
void ConditionDamage::generateDamageList(int32_t amount, int32_t start, std::list<int32_t>& list)
{
	...
	//int32_t sum = 0;
	//double x1, x2;
	start = std::abs(start);//agregado fields como rl
	amount -= start; //agregado fields como rl

	//for (int32_t i = start; i > 0; --i) {
		//int32_t n = start + 1 - i;
		//int32_t med = (n * amount) / start;
	int32_t totalDamage = 0; //agregado fields como rl
	int32_t currentStep = 0; //agregado fields como rl
	...
	while (totalDamage < amount) { //agregado fields como rl
		int32_t turnsCount = getDamageStepValue(++currentStep);
		for (int32_t i = 0; i < turnsCount; i++) {
			totalDamage += currentStep;
			list.push_back(currentStep); //agregado fields como rl
		}
	}
	// initial tick moved
	// list.push_back(start);
	list.reverse(); //agregado fields commo rl
}
