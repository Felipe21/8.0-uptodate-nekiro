...
bool Monster::searchTarget(TargetSearchType_t searchType)//agregado cambio de monster AI
...
void Monster::onThink(uint32_t interval)//agregado cambio de monster AI actulzado revisar
{
	...
	}//agregado cambio de monster AI
}

/*void Monster::onThink(uint32_t interval) //agregado cambio de monster AI
{
...
void Monster::doAttacking(uint32_t interval)
{
	...
	for (const spellBlock_t& spellBlock : mType->info.attackSpells) {
		...
		else if (spellBlock.isMelee && OTSYS_TIME() > (lastMeleeAttack + interval)) {
...
bool Monster::getNextStep(Direction& direction, uint32_t& flags)
{
	...
	if (!walkingToSpawn && (!followCreature || !hasFollowPath) && (!isSummon() || !isMasterInRange)) { //agregado monster iddle behavior
		randomStepping = true;
		//choose a random direction
		result = getRandomStep(getPosition(), direction);
