...
int32_t Player::getDefense() const //AGREGADO OTHIRE CODE reemplazado x el de nostalrius
{
...
/*int32_t defenseSkill = getSkillLevel(SKILL_FIST); //agregado othire remover old code
...
void Player::onWalk(Direction& dir)
{
	...
	//setNextAction(OTSYS_TIME() + getStepDuration(dir)); //agregado removido cast spells while walking
}
...
ReturnValue Player::queryAdd(int32_t index, const Thing& thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	//need an exchange with source? (destination item is swapped with currently moved item)
		//need an exchange with source? (destination item is swapped with currently moved item)
	const Item* inventoryItem = getInventoryItem(static_cast<slots_t>(index)); //modificado para que funcione como en el old school //agregado
...
bool Player::toggleMount(bool mount)
{
	...
	if (mount) {
		...
		//agregado block mount
		//block mount
		//disable mount 
		//edit//ESTO NADA QUE VER CON EL MOUNTS SYSTEM PERO BLOQUEA QUE CIERTOS OUTFITS PUEDAN OCUPAR MOUNT
		if ((playerOutfit->lookType == 590) || (playerOutfit->lookType == 532)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 591) || (playerOutfit->lookType == 533)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 592) || (playerOutfit->lookType == 534)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 593) || (playerOutfit->lookType == 535)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 628) || (playerOutfit->lookType == 628)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 629) || (playerOutfit->lookType == 629)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 630) || (playerOutfit->lookType == 630)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 631) || (playerOutfit->lookType == 631)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType ==632) || (playerOutfit->lookType == 632)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 633) || (playerOutfit->lookType == 633)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 544) || (playerOutfit->lookType == 604)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;
		}
		if ((playerOutfit->lookType == 537) || (playerOutfit->lookType == 603)) {
			sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return false;

		}//ESTO NADA QUE VER CON EL MOUNTS SYSTEM PERO BLOQUEA QUE CIERTOS OUTFITS PUEDAN OCUPAR MOUNT
